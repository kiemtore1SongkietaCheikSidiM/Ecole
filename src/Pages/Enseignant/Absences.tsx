...
const Absences = () => {
  ...
  return (
    <div className=" block dark:bg-slate-800">
      {loading && <LoadingOverlay label="Envoi des absences..." />}
      <ConfirmDialog open={confirmSend} title="Envoyer les absences ?" description={`${todo.length} absence(s) seront enregistrées.`} busy={loading} onCancel={() => setConfirmSend(false)} onConfirm={handleConfirmSend} confirmLabel="Envoyer" />
      <ConfirmDialog open={pendingDelete !== null} title="Supprimer cette ligne ?" description="Cette absence sera retirée de la liste avant l'envoi." onCancel={() => setPendingDelete(null)} onConfirm={() => pendingDelete !== null && supp(pendingDelete)} confirmLabel="Supprimer" />
