...
import { MenuItemEnseignant } from "../../Declarations/Constant";
import { useNavigate } from "react-router-dom";

const user = JSON.parse(localStorage.getItem("user") || "{}");
const SidebarE = ({ sidebarcollaps }: Search) => {
  const navigate = useNavigate();
  return (
    <aside
      className={`flex h-screen flex-col border-r border-slate-200/70 bg-white/85 shadow-lg shadow-slate-200 
             backdrop-blur-xl transition-all duration-300 ease-in-out dark:border-slate-700/70 dark:shadow-slate-950 focus-visible:ring-2 dark:bg-slate-900/80 
